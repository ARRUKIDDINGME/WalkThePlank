...
void ConnectionLoader::doAutoConnect(bool tryEzcashdStart) {
    ...
    // Priority 2: Try to connect to detect ARRRUKIDDING.conf and connect to it.
...
void ConnectionLoader::createZcashConf() {
    ...
    out << "# Autogenerated by Hush WalkThePlank https://myhush.org\n";
    out << "server=1\n";
    out << "rpcuser=hush\n";
...
#ifdef Q_OS_LINUX
    auto confLocation = QStandardPaths::locate(QStandardPaths::HomeLocation, ".komodo/ARRRUKIDDING/ARRRUKIDDING.conf");
#elif defined(Q_OS_DARWIN)
    auto confLocation = QStandardPaths::locate(QStandardPaths::HomeLocation, "Library/Application Support/Komodo/ARRRUKIDDING/ARRRUKIDDING.conf");
#else
    auto confLocation = QStandardPaths::locate(QStandardPaths::AppDataLocation, "../../Komodo/ARRRUKIDDING/ARRRUKIDDING.conf");
#endif
...
#ifdef Q_OS_LINUX
    auto confLocation = QDir(QStandardPaths::writableLocation(QStandardPaths::HomeLocation)).filePath(".komodo/ARRRUKIDDING/ARRRUKIDDING.conf");
#elif defined(Q_OS_DARWIN)
    auto confLocation = QDir(QStandardPaths::writableLocation(QStandardPaths::HomeLocation)).filePath("Library/Application Support/Komodo/ARRRUKIDDING/ARRRUKIDDING.conf");
#else
    auto confLocation = QDir(QStandardPaths::writableLocation(QStandardPaths::AppDataLocation)).filePath("../../Komodo/ARRRUKIDDING/ARRRUKIDDING.conf");
#endif
...
bool ConnectionLoader::verifyParams() {
    ...
    if( QFile( QDir("/Applications").filePath("ARRRUKIDDING-WalkThePlank.app/Contents/MacOS/sapling-output.params") ).exists() && QFile( QDir("/Applications").filePath("./WalkThePlank.app/Contents/MacOS/sapling-spend.params") ).exists() ) {
        qDebug() << "Found params in /Applications/ARRRUKIDDING-WalkThePlank.app/Contents/MacOS";
        return true;
    }

    // this is to support SD on mac inside a DMG
    if( QFile( QDir("./").filePath("ARRRUKIDDING-WalkThePlank.app/Contents/MacOS/sapling-output.params") ).exists() && QFile( QDir("./").filePath("./ARRRUKIDDING-WalkThePlank.app/Contents/MacOS/sapling-spend.params") ).exists() ) {
        qDebug() << "Found params in ./ARRRUKIDDING-WalkThePlank.app/Contents/MacOS";
        return true;
    }
...
std::shared_ptr<ConnectionConfig> ConnectionLoader::autoDetectZcashConf() {    
    ...
    if (zcashconf->port.isEmpty()) zcashconf->port = "48434";
    file.close();

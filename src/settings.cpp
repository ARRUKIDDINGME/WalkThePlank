...
Explorer Settings::getExplorer() {
    ...
    QString explorer = "http://81.169.218.223:40438";
...
PaymentURI Settings::parseURI(QString uri) {
    ...
    if (!uri.startsWith("ARRUKIDDINGME:")) {
        ans.error = "Not a ARRUKIDDINGME payment URI";
        return ans;
    }

    uri = uri.right(uri.length() - QString("ARRUKIDDINGME:").length());

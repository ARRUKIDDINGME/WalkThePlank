...
void RPC::setConnection(Connection* c) {
    ...
    ui->statusBar->showMessage("Ready! Thank you for helping secure the ARRRUKIDDING network by running a full node.");
    ...
    conn->doRPC(makePayload(method), [=] (const json& reply) {   
	    ...
	    // TODO: use getinfo.synced
            ...
            QString statusText = QString() %
                ...
                " ARRRUKIDDING" //% QString::number( (double) Settings::getInstance()->getZECPrice() ) %
...
void RPC::checkForUpdate(bool silent) {
    ...
    QUrl cmcURL("https://api.github.com/repos/MyHush/WalkThePlank/releases");
            ...
            if (reply->error() == QNetworkReply::NoError) {
                ...
                if (maxVersion > currentVersion && (!silent || maxVersion > maxHiddenVersion)) {
                    ...
                    if (ans == QMessageBox::Yes) {
                        QDesktopServices::openUrl(QUrl("https://github.com/MyHush/WalkThePlank/releases"));
                    } else {
...
void RPC::shutdownZcashd() {
    ...
    connD.status->setText(QObject::tr("Please enhance your calm and wait for WalkThePlank to exit"));
